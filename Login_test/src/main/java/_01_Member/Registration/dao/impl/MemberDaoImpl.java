...
public class MemberDaoImpl implements MemberDao {
	...
	public List<MemberBean> checkAccountPassword(String account, String password) {
		Session session = factory.getCurrentSession();
		String sql = "SELECT * FROM MembraneRegistration m WHERE m.cusAccount = account and m.cusPassword = :password";
		List<MemberBean> list = null;
		list = session.createQuery(sql).setParameter("account", account).setParameter("password", password).getResultList();
		return list;
	}
