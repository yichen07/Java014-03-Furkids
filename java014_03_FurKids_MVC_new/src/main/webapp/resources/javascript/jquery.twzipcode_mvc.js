    ...
    TWzipcode.prototype = {
        ...
        reset: function (container, obj) {
            var self = this,
                ...
                county = '',
                list = {
                    'county': '<form:options value="">縣市</form:options>',
                    'district': '<form:options value="">鄉鎮市區</form:options>'
                },
                tpl = [];
            ...
            default:
                ...
                for (county in data) {
                    if ('undefined' !== typeof data[county] && -1 === opts.hideCounty.indexOf(county)) {
                        tpl.push('<form:options value="' + county + '">' + county + '</form:options>');
                    }
                }
        ...
        bindings: function () {
            ...
            wrap.county.on('change.twzipcode', function () {
                ...
                if (val) {
                    if (true === opts.zipcodeIntoDistrict) {
                        for (district in data[val]) {
                            if ('undefined' !== typeof data[val][district] &&
                                ...
                                tpl.push('<form:options value="' + district + '">');
                                tpl.push(data[val][district] + ' ' + district);
                                tpl.push('</form:options>');
                            }
                        }
                    } else {
                        for (district in data[val]) {
                    ...
                    wrap.county.find('form:options:first').prop('selected', true);
                    self.reset('district');
                }
